...
const DICT: Dict = {
  ...
  "titlebar.open_explorer":       ["资源管理器", "Explorer"],
  "titlebar.open_idea":           ["IDEA", "IDEA"],
  "titlebar.open_terminal":       ["终端", "Terminal"],
  "titlebar.open_vscode":         ["VS Code", "VS Code"],
  "titlebar.choose_idea":         ["选择 IDEA 可执行文件…", "Choose IDEA executable..."],
  "titlebar.choose_vscode":       ["选择 VS Code 可执行文件…", "Choose VS Code executable..."],
  "titlebar.open_failed":         ["打开失败", "Failed to open"],
  "titlebar.choose_app":          ["选择可执行文件", "Choose executable"],
  "titlebar.choose_app_failed":   ["选择可执行文件失败", "Failed to choose executable"],
  ...
  // S16（plan-41-197）：细化模板提示词——写明角色、步骤与输出要求；新增「排查线上报错」「安全重构」。
  "workspace.subtitle":           ["今天想让我帮你做点什么？", "What can I help you with today?"],
  "workspace.quick_fix":          ["修复报错", "Fix a bug"],
  "workspace.quick_fix_prompt":   ["帮我修复以下报错。要求：① 先定位根因（给出证据链，如日志 / 代码位置）；② 给出最小修复方案；③ 用测试或运行验证修复有效，并说明如何回归验证：\n", "Help me fix the following error. Requirements: (1) locate the root cause with an evidence chain (logs / code); (2) give the minimal fix; (3) verify with tests or a run, and describe regression steps:\n"],
  "workspace.quick_triage":       ["排查线上报错", "Triage a live error"],
  "workspace.quick_triage_prompt": ["线上出现以下报错，请按「复现 → 定位 → 修复 → 验证」推进：先确认复现条件，再定位根因并给出证据；修复只做最小改动，不顺手重构无关代码；最后给出验证方式：\n", "A live error occurred. Work through reproduce → locate → fix → verify: confirm the repro first, find the root cause with evidence, apply the minimal fix (no unrelated refactors), then show how to verify:\n"],
  "workspace.quick_test":         ["写单测", "Write tests"],
  "workspace.quick_test_prompt":  ["为以下代码编写单元测试。要求：覆盖正常路径与边界情况（空值 / 极值 / 异常分支），每个用例用注释说明意图，最后运行测试并报告结果：\n", "Write unit tests for the following code. Cover normal paths and edge cases (nulls, extremes, error branches), comment each case's intent, then run the tests and report results:\n"],
  "workspace.quick_review":       ["代码审查", "Code review"],
  "workspace.quick_review_prompt": ["请审查以下代码。按「正确性风险 / 边界与异常 / 可读性 / 性能」四类给出发现，每条给出证据（文件:行号）与具体修改建议，不要泛泛而谈：\n", "Review the following code. Report findings under four groups — correctness risks / boundaries and errors / readability / performance — each with evidence (file:line) and a concrete suggestion, not vague advice:\n"],
  "workspace.quick_explain":      ["解释代码", "Explain code"],
  "workspace.quick_explain_prompt": ["请解释以下代码：先一句话概括作用，再按执行流程说明关键实现思路与设计取舍，最后指出容易误解的点：\n", "Explain the following code: summarize its purpose in one sentence, walk through the flow and key design trade-offs, then point out easily misunderstood parts:\n"],
  "workspace.quick_refactor":     ["安全重构", "Safe refactor"],
  "workspace.quick_refactor_prompt": ["重构以下代码以提升可读性与结构，**保持对外行为完全不变**。开始前先列出重构计划与风险点；重构后运行类型检查 / 测试验证，再分步说明每处改动：\n", "Refactor the following code for readability and structure while keeping external behavior identical. List the plan and risks first, verify with type checks / tests, then explain each change:\n"],
  ...
  "composer.mode_title":          ["执行模式", "Execution Mode"],
  "composer.mode_agent":          ["智能体模式", "Agent Mode"],
  "composer.mode_plan":           ["计划模式", "Plan Mode"],
  "composer.mode_readonly":       ["只读模式", "Read Only"],
  "composer.approval_title":      ["权限模式", "Permission Mode"],
  "composer.approval_ask":        ["询问审批", "Ask Approval"],
  "composer.approval_ask_desc":   ["执行命令、修改工作区外文件或访问网络前，始终询问", "Always ask before running commands, writing outside the workspace, or accessing the network"],
  "composer.approval_auto":       ["自动审批", "Auto Approve"],
  "composer.approval_auto_desc":  ["仅在检测到潜在风险时询问", "Only ask when a potential risk is detected"],
  "composer.approval_full":       ["完全访问", "Full Access"],
  "composer.approval_full_desc":  ["不再询问，可自由访问你的文件、终端和网络", "Never ask; free access to your files, terminal and network"],
  ...
  "settings.tab.scheduled":       ["自动化", "Automations"],
  ...
  "settings.ps.pets":       ["桌面宠物：实时反映任务执行状况；多任务并行时显示数量、步骤与耗时", "Desktop pet: live task status; multi-task count, steps & elapsed time when running in parallel"],
  ...
  "settings.ps.index":      ["每个工作目录独立开启代码符号索引，AI 可快速定位函数与文件结构", "Per-workspace code symbol index for fast symbol navigation"],
  ...
  "settings.ps.scheduled":  ["自动化任务的创建、编辑与自动触发配置", "Automation task creation, editing & auto-trigger"],
  ...
  "gp.retry_intervals_desc":      ["每次重试前的等待秒数，逗号分隔；默认 10,20,30 表示依次等待", "Seconds to wait before each retry, comma separated; default 10,20,30"],
