...
export function RulesPanel() {
  ...
  return (
    <div>
      {/* S11（plan-41-197）：头部改用统一类名（原为内联样式拼装），与全局排版语言一致 */}
      <div className="rules-head">
        <span className="rules-head-hint">扫描项目根目录下常见 AI 软件的规则文档，并按来源启用 / 停用</span>
          ...
          <div key={s.source} className="settings-resource-item">
            <div className="settings-resource-info">
              ...
              <div className="settings-resource-desc">
                {/* S11：路径标签去除强调色（“多个重点色”的主要来源），统一为中性次级色 */}
                {scanned.filter((x) => x.source === s.source).map((x) => (
                  <span key={x.path} className="settings-resource-tag" title={x.path}>{x.path}</span>
                ))}
