...
type Tab = "general" | "models" | "skills" | "mcp" | "scheduled" | "policy" | "hooks" | "memory" | "diagnostics" | "about";
...
function SkillsPanel() {
  ...
  const load = useCallback(async () => { try { setItems(await api.listSkills()); } catch {} }, []);
  useEffect(() => { load(); }, [load]);
  return (<div><div style={{ display: "flex", gap: 8, justifyContent: "flex-end", marginBottom: 12 }}><button className="btn btn-ghost btn-sm" onClick={load}><IconRefresh size={13} /> 刷新扫描</button></div><div className="settings-resource-list">{items.map((s) => (<div key={s.id} className="settings-resource-item"><div className="settings-resource-info"><div className="settings-resource-name">{s.display_name || s.name}</div><div className="settings-resource-desc"><span>{s.description || "无描述"}</span><span className="settings-resource-tag">{s.source}</span></div></div><div className="settings-resource-actions"><Sw checked={s.is_active} onChange={async (v) => { try { await api.updateSkill(s.id, { is_active: v }); load(); } catch {} }} /><button className="btn btn-ghost btn-xs" onClick={async () => { if (confirm("删除？")) { try { await api.deleteSkill(s.id); load(); } catch {} } }}><IconX size={12} /></button></div></div>))}{items.length === 0 && <div className="navpage-empty">暂无技能</div>}</div></div>);
}
...
const TABS: { k: Tab; l: string }[] = [
  ...
  { k: "mcp", l: "MCP" }, { k: "scheduled", l: "定时任务" }, { k: "policy", l: "执行策略" },
